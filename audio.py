...
bot = commands.Bot(command_prefix=get_prefix,description='A music bot fro discord SMX3K')
                ...
                if len(bot.voice_client_in(before.server).channel.voice_members) <= 1: #there is only bot in voice channel
                    if player_status[before.server.id]==True:
                        ...
                        if len(bot.voice_client_in(before.server).channel.voice_members) <= 1:
                            ...
                            await bot.send_message(discord.Object(id=rq_channel[before.server.id]),"**وقف موسيقى لأنو مافي شخص في روم  `{}`**".format(before.voice.voice_channel))
